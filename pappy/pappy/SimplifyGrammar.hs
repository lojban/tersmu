...
simplifyGrammar g@Grammar { grammarNonterminals = nonterms, grammarTops = topnts } = g { grammarNonterminals = iteropt topnts nonterms }
...
peephole g = nonterms g where
	...
	rule (RuleError r s) = (RuleError r' s, sr) where (r', sr) = rule r
...
collapse tops g = scandups [] (\n -> n) g where
	...
	-- under the nonterminal renaming function f.
        equivRule f RulePos       RulePos       = True
	...
	rebuildRule f (RuleError r s) = RuleError (rebuildRule f r) s
...
inline tops g = scan g where
	...
	refsRule n (RuleError r _) = refsRule n r
	...
	rebuildRule ns RulePos = RulePos
	...
	rebuildRule ns (RuleError r s) = RuleError (rebuildRule ns r) s
